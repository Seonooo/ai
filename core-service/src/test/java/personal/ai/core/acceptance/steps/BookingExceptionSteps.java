...
public class BookingExceptionSteps {
    ...
    private Long currentScheduleId = 1L;
    ...
    public void 존재하지_않는_좌석_ID로_예약을_요청한다() {
        ...
        currentUserId = 1L;
        currentQueueToken = testUtility.issueActiveQueueToken(DEFAULT_CONCERT_ID, currentUserId);

        log.info(">>> When: POST /api/v1/reservations - 존재하지 않는 좌석 - seatId={}", invalidSeatId);
        lastHttpResponse = httpAdapter.reserveSeat(currentScheduleId, invalidSeatId, currentUserId, currentQueueToken);
    }

    @Then("요청된 예약이 거부된다")
    public void 요청된_예약이_거부된다() {
        // 존재하지 않는 좌석: 리소스를 찾을 수 없음
        assertThat(lastHttpResponse.statusCode()).isEqualTo(HttpStatus.NOT_FOUND.value());
    ...
    public void 다른_사용자가_생성한_예약이_존재한다() {
        ...
        otherUserId = testUtility.createUser("OtherUser");
        Long seatId = testUtility.createSeat(currentScheduleId, "B1", SeatStatus.AVAILABLE);

        // 다른 사용자로 예약 생성
        otherReservationId = testUtility.createReservation(null, otherUserId, seatId, ReservationStatus.PENDING);
        log.info(">>> Given: 타인 예약 생성 - reservationId={}, userId={}", otherReservationId, otherUserId);
    }

    @When("해당 예약 조회를 요청한다")
    public void 해당_예약_조회를_요청한다() {
        currentUserId = 1L; // 현재 로그인한 사용자
        log.info(">>> When: GET /api/v1/reservations/{} - 타인 예약 조회 시도", otherReservationId);
        lastHttpResponse = httpAdapter.getReservation(otherReservationId, currentUserId);
    }

    @Then("조회가 거부된다")
    public void 조회가_거부된다() {
        assertThat(lastHttpResponse.statusCode()).isEqualTo(HttpStatus.FORBIDDEN.value()); // 403 Forbidden Expected
    ...
    public void 내가_생성한_예약이_이미_확정_상태이다() {
        testUtility.createScheduleIfNotExists(currentScheduleId);
        Long myUserId = 1L;
        Long seatId = testUtility.createSeat(currentScheduleId, "C1", SeatStatus.RESERVED);

        myReservationId = testUtility.createReservation(null, myUserId, seatId, ReservationStatus.CONFIRMED);
        log.info(">>> Given: 확정된 예약 생성 - reservationId={}", myReservationId);
    }
    ...
    public void 내가_생성한_예약이_만료되었다() {
        testUtility.createScheduleIfNotExists(currentScheduleId);
        Long myUserId = 1L;
        Long seatId = testUtility.createSeat(currentScheduleId, "D1", SeatStatus.AVAILABLE);

        // 예약 생성 후 강제 만료
        myReservationId = testUtility.createReservation(null, myUserId, seatId, ReservationStatus.PENDING);
        testUtility.expireReservationImmediately(myReservationId);

        log.info(">>> Given: 예약 만료 처리 완료 - reservationId={}", myReservationId);
    }

    @When("해당 예약에 대해 결제를 요청한다")
    public void 해당_예약에_대해_결제를_요청한다() {
        Long myUserId = 1L;
        log.info(">>> When: POST /api/v1/payments - reservationId={}", myReservationId);
    ...
    public void 요청된_결제가_거부된다() {
        ...
        assertThat(lastHttpResponse.statusCode()).isEqualTo(HttpStatus.BAD_REQUEST.value());
    ...
    public void 내가_생성한_예약이_존재한다() {
        testUtility.createScheduleIfNotExists(currentScheduleId);
        Long myUserId = 1L;
        Long seatId = testUtility.createSeat(currentScheduleId, "P1", SeatStatus.AVAILABLE);

        myReservationId = testUtility.createReservation(null, myUserId, seatId, ReservationStatus.PENDING);
        log.info(">>> Given: 내 예약 생성 - reservationId={}", myReservationId);
    }
...
//    }
    ...
    public void 해당_예약에_대해_내가_결제를_요청한다() {
        ...
        Long myUserId = 1L;
        log.info(">>> When: POST /api/v1/payments - 타인 예약 결제 시도 - reservationId={}", otherReservationId);
        lastHttpResponse = httpAdapter.processPayment(
                otherReservationId,  // 타인의 예약 ID
                myUserId,  // 요청자는 나
                new java.math.BigDecimal("50000"),
                "CREDIT_CARD",
                DEFAULT_CONCERT_ID);
    ...
    public void 요청된_결제가_사용자_불일치로_거부된다() {
        ...
        assertThat(lastHttpResponse.statusCode()).isEqualTo(HttpStatus.FORBIDDEN.value());
